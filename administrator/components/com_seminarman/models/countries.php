...
class seminarmanModelCountries extends JModelLegacy
{
    var $_data = null;
    ...
    function __construct()
    {
        ...
        $this->childviewname = 'countries';

        $limit = $mainframe->getUserStateFromRequest('global.list.limit', 'limit', $mainframe->getCfg('list_limit'), 'int');
        $limitstart = $mainframe->getUserStateFromRequest('com_seminarman' . '.sman_lands_limitstart', 'sman_lands_limitstart', 0, 'int');

        $limitstart = ($limit != 0 ? (floor($limitstart / $limit) * $limit) : 0);

        $this->setState('limit', $limit);
        $this->setState('sman_lands_limitstart', $limitstart);
    }
    ...
    function getData()
    {
        ...
        if (empty($this->_data))
        {
            ...
            $this->_data = $this->_getList($query, $this->getState('sman_lands_limitstart'), $this->getState('limit'));
        }
    ...
    function getPagination()
    {
        ...
        if (empty($this->_pagination))
        {
            ...
            $this->_pagination = new JPagination($this->getTotal(), $this->getState('sman_lands_limitstart'), $this->getState('limit'), 'sman_lands_');
        }
    ...
    function _buildQuery()
    {
        ...
        $query->from( '#__seminarman_country AS a' );
