...
class seminarmanModelCountry extends JModelLegacy
{
    ...
    function delete($cid = array())
    {
        if ( count( $cid ) ){
        	...
        	$query->where( 'id_country IN ( ' . $cids . ' ) OR bill_id_country IN (' . $cids . ')' );
            ...
            if ($relatedRecords > 0)
            {
                JError::raiseWarning('ERROR_CODE', JText::_('There are records of tutor assigned to selected country. Please delete the related records first.'));
                return false;
            }
